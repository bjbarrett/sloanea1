...
techmeans <- c(8,10,12,14 ) # list of tech means for successes from data

techvar <- c( 4,4,4,4 ) #list of tech variances for successes from data

#techprsucceed <- c(0.511,0.378,0.885,0.665) #probability technique leads to success


#parameter sims
fc.sim <- log(0)				#frequency dependecy parameter on log scale
phi.sim <- logit(.15) ## attraction updating parameter on log-odds scale
chi.sim <- logit(.1) ## attraction updating parameter on log-odds scale
gamma.sim <- logit(.7) ## influence of social info parameter on log-odds scale
k.lambda <- 25           ##sensitivity to individual payoffs
beta.p <- 2         ##contribution of payoff bias cues

#varying effects offsets for individuals
gamma.sim_i <- rnorm( n , mean=0 , sd=.5 ) 
phi.sim_i <- rnorm( n , mean=0 , sd=.5 ) 
fc.sim_i <- rnorm( n , mean=0 , sd=.5 )
chi.sim_i <- rnorm( n , mean=0 , sd=.5 ) 
beta.p_i <- rnorm( n , mean=0 , sd=.5 )
chi.sim_i <- rnorm( n , mean=0 , sd=.5 )
...
#simulate data with moving window
dsim_s <- data.frame( i=0 , bout=0 , tech=0 , y1=0 , y2=0, y3=0 , y4=0 , s1=0 , s2=0 , s3=0 , s4=0, ps1=0 , ps2=0 , ps3=0 , ps4=0 , A1=0 , A2=0 , A3=0 , A4=0 , B1=0 , B2=0 , B3=0 , B4=0)
therow <- 1
AC <- matrix(1,ncol=4,nrow=n) #attraction scores for each tech
BC <- matrix(1,ncol=4,nrow=n) #social info window

#AC[,3] <- 0 #all have equal prob of other behaviors, except option 3
AC[1,] <- c(.5,.5,.5,.5) #equal probs
BC[1,] <- c(.5,.5,.5,.5) #equal probs

S1 <- S2 <- S3 <- S4 <- rep(0,n+1) # num of individuals choosing each tech in previous bout
PS1 <- PS2 <- PS3 <- PS4 <- rep(0,nbouts+1) # empty vector for cues observed in previous round
...
for ( r in 1:nbouts ) {
    for ( i in 1:n ) {  
        ...
        my.phi <- logistic( phi.sim + phi.sim_i[i] ) #personal info memory weight  parameter for individual i
        my.chi <- logistic( chi.sim + chi.sim_i[i] ) #personal info memory weight  parameter for individual i
        ...
        if ( r > 1 ) {
            if (sum( prtech_su ) > 0 ) {
                ...
                #lin_mod[1] <- 1 #// aliased outcome i commented this out and need to revisit it

                #// compute frequency cue
                #for ( j in 1:4 ){ s_temp[j] <- prtech_su[j]^my.fconf} #conformity component
                for ( j in 1:4 ){ s_temp[j] <- 1} #makes this all 1, can modify so cue bias is the only thing that appears
                for ( j in 1:4 ){lin_mod[j] <- lin_mod[j] * s_temp[j]}
                for ( j in 1:4 ){BC[i,j] <- (1-my.chi)*BC[i,j] + my.chi*lin_mod[j]}
                prtech_s <- BC[i,]/sum(BC[i,])
...
# update attractions
        yields <- rep(0,4)
        #yields[tech] <- 1 #gives a payoff of 1 if behavior performed
        yields[tech] <- yield #gives a payoff of 1 if behavior performed
        for (k in 1:4){
            AC[i,k] <- (1-my.phi)*AC[i,k] + my.phi*yields[k]
        }

        dsim_s[therow,] <- c( i , r , tech , yields[1] , yields[2] , yields[3] , yields[4] , S1[r] , S2[r] , S3[r] , S4[r], PS1[r] , PS2[r] , PS3[r] , PS4[r] , AC[i,1] , AC[i,2] , AC[i,3] , AC[i,4], BC[i,1] , BC[i,2] , BC[i,3] , BC[i,4])
        therow <- therow + 1
    } #i
...
#plot simulated data
plot(s1/n ~ bout, data=dsim2, col="red" , ylim=c(0,1) , pch=19 , xlab="Time (Foraging Bouts)" , ylab="Proportion of Individuals Choosing Option" , xlim=c(2,70) )
...
# par(mfrow=c(3, 2), oma = c(0, 0, 0, 0) , mar=c(3,.5,2,.5))
# par(cex = 0.6)
# par(tcl = -0.25)
# par(mgp = c(2, 0.6, 0))
...
# phi.pred <- rep(0,n)
# for(i in 1:n){phi.pred[i] <- mean(logistic(post$mu[,1] + post$a_id[,i,1]))}
# plot(phi.k,phi.pred , pch=19 , col="slateblue" , xlim=c(0,0.6) , ylim=c(0,0.6) )
# abline(a = 0, b = 1)


# emaweights<-function(m, alpha)
# {
#   i<-1:m
...
# # barplot(emaweights(20, 0.1) )
# # barplot(emaweights(20, 0.4) ,col="violet" , add=TRUE)
# # barplot(emaweights(20, 0.7) ,col="turquoise" , add=TRUE)

# test <- rbind (emaweights(20, 0.1) , emaweights(20, 0.4) , emaweights(20, 0.7) )
# barplot(test, beside=TRUE, col=brewer.pal(3,"Spectral") , ylab="weight at each timestep" , xlab="<- time <-")
