...
d <- read.csv("~/Dropbox/sloanea/ST2003to2012_Oct11aabbccrav_2018.csv", na.strings = "" , stringsAsFactors=FALSE) #including 10 monkeys
...
###check date formate and choose below correctly because excel sucks
d$date2 <- dmy(as.factor(d$date))  #excel might fuck this up if you open csv
d$date <- ymd(as.factor(d$date))  #excel might fuck this up if you open csv
...
for( nobs in 1:nrow(d) ){
    for (i in 1:nrow(foc)){
        if ( grepl(foc[i,1],d[nobs,"rav"])==TRUE || grepl(foc[i,1],d[nobs,"AA"])==TRUE || grepl(foc[i,1],d[nobs,"BB"])==TRUE || grepl(foc[i,1],d[nobs,"CC"])==TRUE){
...
ageyears( d$dob[1] , d$date[1])
###this orders in monkey order
library(chron)
d <- d[order(d$mono_index,d$timedate),]

write.csv(d, "ST_14_aabbccrav_11Oct2018.csv")
###migration dataset
dm<- subset(d,male==1)
