...
model {
    ...
    for ( i in 1:N ) {
        ...
        if ( bout[i]==1 ) {
            ...
            fconf = exp( mu[3] + a_id[id[i],3] + b_age[3]*age[i]);                         // strength of frequency dependence
            beta_coho = ( mu[4] + a_id[id[i],4] + b_age[4]*age[i]);                             //contribution of cohort bias
            beta_age = ( mu[5] + a_id[id[i],5] + b_age[5]*age[i]);                             //contribution of age bias
            beta_kin = ( mu[6] + a_id[id[i],6] + b_age[6]*age[i]);                             //contribution of kin bias
