   ...
   //priors
    ...
    for ( i in 1:n_obs ) {
        ...
        if ( bout[i]==1 ) {
            ...
            gamma[id[i]] = inv_logit(  I[id[i],3] + G[group_index[i],3]  + S[sex_index[i] , 3] + bA[sex_index[i] , 3]*logage[i] );
            fc[id[i]] =  exp( I[id[i] , 4] + G[group_index[i] , 4]  + S[sex_index[i] , 4]  + bA[sex_index[i] , 4]*logage[i] ) ;
        }
