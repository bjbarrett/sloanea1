...
parameters {
    ...
    vector[6] b_m;                          // migration per parameter intercepts
...
model {
    ...
    for ( i in 1:N ) {
        ...
        if ( bout[i]==1 ) {
            ...
            phi = inv_logit( mu[1] + a_id[id[i],1] + (mu[7] + a_id[id[i],7])*age[i]  + b_m[1]*postmig[i] );  //attraction to new experience
            gamma = inv_logit( mu[2] + a_id[id[i],2] + (mu[8] + a_id[id[i],8])*age[i] + b_m[2]*postmig[i]); //weight given to social info
            fconf = exp( mu[3] + a_id[id[i],3] + (mu[9] + a_id[id[i],9])*age[i] + b_m[3]*postmig[i]);                         // strength of frequency dependence
            beta_coho = ( mu[4] + a_id[id[i],4] + (mu[10] + a_id[id[i],10])*age[i] + b_m[4]*postmig[i]);                             //contribution of cohort bias
            beta_age = ( mu[5] + a_id[id[i],5] + (mu[11] + a_id[id[i],11])*age[i] + b_m[5]*postmig[i]);                             //contribution of age bias
            beta_kin = ( mu[6] + a_id[id[i],6] + (mu[12] + a_id[id[i],12])*age[i] + b_m[6]*postmig[i]);                             //contribution of kin bias
