data {
    ...
    real q[N,K];        // observed cohort social variables of techs 1-K
    real s[N,K];        // observed number of ttimes observing behaviors
    int bout[N];        // bout or fruit per id
    int id[N];          // player id
    int N_effects;      // number of learning parameters to estimate
    real age[N];        // standardize age of individual at an obsertvation
}
parameters {
...
model {
    ...
    for ( i in 1:N ) {
        ...
        if ( bout[i]==1 ) {
            ...
            beta =  mu[3] + a_id[id[i],3] + (mu[6] + a_id[id[i],6])*age[i] ;           //contribution of cue bias
        }
        ...
        if ( bout[i] > 1 ) {
            if (sum( s[i] ) > 0 ) {                                       //only use social info when it is observed
                ...
                target += ( log( (1-gamma)*exp(logPrA) + gamma*PrS ) ); //this is wonky and can be made more efficient
...
generated quantities{
    ...
    dev=0;
        ...
        if ( bout[i]==1 ) {
            ...
            phi = inv_logit( mu[1] + a_id[id[i],1] + (mu[4] + a_id[id[i],4])*age[i] );  //attraction to new experience
            gamma = inv_logit( mu[2] + a_id[id[i],2] + (mu[5] + a_id[id[i],5])*age[i] ); //weight given to social info
            beta =  mu[3] + a_id[id[i],3] + (mu[6] + a_id[id[i],6])*age[i] ;                             //contribution of cohort bias
